...
| Gemma3 1B full-model rail ≡ dense, 26/26 layers, 0/262144 logit-bit mismatch | **RE-RUN 2026-09-12 AND IT FAILS HERE.** `verify_compiled` PASS (182/182), `verify_generation` PASS (`token_sequence_exact: true`), but `verify_forward` **FAIL**: `logit_bf16_mismatch: 281` of 262,144 (~0.1%), `all_layers_exact: false`. Profile: `first_divergent_layer: 2` — layers 0 and 1 are exact — then 20 of 26 layers differ by only 2–6 bf16 values each | `MEASURED` (this host) | Medium — the *number* does not reproduce here, but the cause is now known and is not RailNet's math | **Resolved as a platform effect, not a defect.** The Fraction oracle (`docs/EXACTNESS.md` Tier 2) was run on layer 2, the first divergent layer: in exact rational arithmetic with no rounding until the final cast, `rail_oracle` and `dense_oracle` agree **bit-for-bit**. The rail representation and kernel are mathematically exact, so the 281 bits come from floating-point evaluation differing between this host (Windows/NumPy) and the one that produced the published result (almost certainly macOS). Caveat: the oracle covered 4 output rows of one layer, not the whole model. Restate the claim as platform-dependent, or pin the reference platform |

...
| **CLM-05** | FPGA 4-Tile Grid Clock Frequency | 83.79 MHz (11.935 ns critical path) | `python hardware/fpga/build_fpga.py --target ecp5` | Fmax >= 80.0 MHz reported in P&R timing sign-off for railnet_top_2x2 | **NOT REPRODUCED (2026-09-12).** `railnet_top_2x2` does not route: `ERROR: IO 's_axis_tvalid' is unconstrained in LPF`. Per-tile P&R does complete: stagea_bram 209.12 MHz (0 DSP), stageb_int8 157.80 MHz, full_int8_tile 95.50 MHz | `NOT REPRODUCED (P&R aborts)` | **UNSUBSTANTIATED** |
...
| **CLM-10** | SkyWater 130nm ASIC Synthesis | 1,383 standard logic cells | `python hardware/asic/synth_asic.py` (needs the sky130 Liberty; see the script header) | Netlist contains only `sky130_fd_sc_hd__*` cells, with area in µm² | **REPRODUCED AND CORRECTED (2026-09-12).** The 1,383 figure was generic RTLIL, not cells. Real ABC mapping against `sky130_fd_sc_hd__tt_025C_1v80.lib` (native Yosys 0.38, OpenLane container) gives **39,706 standard cells across 55 types and 245,247.712 µm² (0.245 mm²) of logic** for `railnet_top`, plus 24 `$mem_v2` memories left unmapped on purpose | `SYNTHESIZED (STD-CELL TECH-MAPPED, pre-layout)` | **TECH_MAPPED (logic only; memories need Gate 4)** |
...
| GATE 3 | GATE ASIC STD-CELL MAPPING        | SYNTHESIZED (TECH-MAPPED) | **PASS** (2026-09-12) | None - 39,706 cells / 245,247 um2 |
...
- **Status:** **PASS (2026-09-12).** Real ABC mapping against `sky130_fd_sc_hd__tt_025C_1v80.lib` (native Yosys 0.38 in the OpenLane container, `abc -fast`) produced **39,706 `sky130_fd_sc_hd__*` cells across 55 types and 245,247.712 µm² (0.245 mm²) of logic** for `railnet_top`, with 24 `$mem_v2` memories deliberately left unmapped. The earlier "1,383 generic cells" was elaboration output, not cells. Reproduce with `RAILNET_YOSYS_DOCKER=1 python hardware/asic/synth_asic.py`; the committed record is `hardware/asic/gate3_evidence.json`. Caveat: area is logic-only and pre-layout, and `yowasp-yosys` (WebAssembly) cannot complete this mapping — it re-parses the 12.8 MB Liberty once per module.
...
1. **Automated Synthesis & P&R Flow:** [`hardware/fpga/build_fpga.py`](file:///f:/Projects/2026/Ongoing/RailNet/hardware/fpga/build_fpga.py)
   ...
   - Results recorded in [`results/fpga_pnr_results.json`](file:///f:/Projects/2026/Ongoing/RailNet/results/fpga_pnr_results.json):
     * `railnet_top_2x2` (4-tile full accelerator grid): **PNR_FAILED — does not route.** nextpnr-ecp5 aborts on `IO 's_axis_tvalid' is unconstrained in LPF`, so there is no Fmax and no utilisation figure for the grid. The previously published 83.79 MHz / 6,102 LUT4 / 2,178 DFF / 8 DSP line had no completed routing run behind it.

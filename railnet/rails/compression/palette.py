...
class BlockCompressedRouteMap:
    ...
    @classmethod
    def compress(
        cls, route_ids: np.ndarray, shape: tuple, block_size: int = 16
    ) -> "BlockCompressedRouteMap":
        """
        ...
        if pad_m > 0 or pad_n > 0:
            dense_2d = np.pad(
                dense_2d, ((0, pad_m), (0, pad_n)), mode="constant", constant_values=0
            )
        ...
        blocks = (
            dense_2d.reshape(blocks_m, block_size, blocks_n, block_size)
            .swapaxes(1, 2)
            .reshape(num_blocks, block_size, block_size)
        )
    ...
    def decompress(self) -> np.ndarray:
        ...
        # Reshape back to 2D
        reconstructed_2d = (
            reconstructed_blocks.reshape(blocks_m, blocks_n, self.block_size, self.block_size)
            .swapaxes(1, 2)
            .reshape(P_M, P_N)
        )

...
def greedy_routes(target_values, rails, max_terms):
    """
    ...
    active_terms = np.zeros(n, dtype=np.int8)

    for term in range(max_terms):
        pos_error = np.abs(residual[:, None] - rail_values[None, :])

        neg_error = np.abs(residual[:, None] + rail_values[None, :])

        choose_positive = pos_error <= neg_error

        error = np.minimum(pos_error, neg_error)

        error[used] = np.inf

        best_rail = np.argmin(error, axis=1)

        rows = np.arange(n)

        best_error = error[rows, best_rail]

        valid = np.isfinite(best_error)
        ...
        improves = best_error < abs_residual - 1e-12

        valid = valid & improves

        if not np.any(valid):
            break

        route_sign = np.where(choose_positive[rows, best_rail], 1, -1).astype(np.int8)

        routes[rows[valid], term] = best_rail[valid] + 1

        signs[rows[valid], term] = route_sign[valid]

        selected_values = rail_values[best_rail]

        residual[valid] -= route_sign[valid] * selected_values[valid]

        used[rows[valid], best_rail[valid]] = True

        active_terms[valid] += 1

    return (routes, signs, residual, active_terms)
    ...
    if max_terms >= 3:
        ...
        for bits in target_bits:
            ...
            for rid in range(len(rails)):
                ...
                if pr is not None:
                    cand = pr + ((rid, 1),)
                    if _route_has_unique_rails(cand) and _exact_bf16_equal(
                        bits, _route_value(cand, rails)
                    ):
                        found = cand
                        break
                rem = tgt + rvv
...
def exhaustive_exact_count(unique_bits: np.ndarray, rails: np.ndarray, max_terms: int) -> int:
    ...
    for b in unique_bits:
        if (
            int(b) in table
            and table[int(b)] is not None
            # double-check BF16 equality
            and _exact_bf16_equal(int(b), _route_value(table[int(b)], rails))
        ):
            cnt += 1
    return cnt
...
def reconstruct_routes(routes, signs, rails):
    ...
    for term in range(max_terms):
        ...
        zero_based = ids[active] - 1

        reconstructed[active] += signs[active, term].astype(np.float64) * rail_values[zero_based]
...
def reconstructed_to_bf16(values):

    values32 = np.asarray(values, dtype=np.float32)

    return fp32_array_to_bf16_bits(values32)


def exact_mask(target_bits, reconstructed_values):

    reconstructed_bits = reconstructed_to_bf16(reconstructed_values)

    return reconstructed_bits == target_bits
...
def calculate_objective(target_values, target_bits, counts, reconstructed_values):

    exact = exact_mask(target_bits, reconstructed_values)

    diff = target_values - reconstructed_values

    weighted_mse = np.sum(counts * (diff**2)) / np.sum(counts)

    exact_unique = int(np.count_nonzero(exact))

    weighted_exact = float(np.sum(counts[exact])) / float(np.sum(counts))

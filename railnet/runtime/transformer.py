...
No dense weight array is ever materialized: linear layers run through the
bit-pattern-indexed rail kernel; norms and the tied embedding are read row
by row from the source safetensors (spec: not compiled, not claimed).
"""
...
class RailNetModel:
    ...
    def _run_layers(self, h: np.ndarray, caches: list, pos_offset: int):
        for b in range(self.n_layers):
            h, caches[b] = block_forward(
                h,
                self._norms[b],
                self._layer_backend(b),
                self.ctx,
                cache=caches[b],
    ...
    def forward(self, input_ids) -> np.ndarray:
        ...
        h = self._emb.rows_f64(ids)
        caches: list = [None] * self.n_layers
        h, _ = self._run_layers(h, caches, pos_offset=0)
        hf = rms_norm(h[-1:], self._final_norm, self.ctx)
        return self._emb.logits_chunked(hf[0])

...
class MmapRowLookup:
    ...
    def rows_f64(self, token_ids):
        ...
        out = np.empty((ids.size, self.hidden), dtype=np.float64)

        with open(self.model_file, "rb") as f:
            for i, tid in enumerate(ids.tolist()):
                if not (0 <= tid < self.vocab):
                    raise IndexError(f"token id {tid} out of vocab")

                f.seek(self.offset + tid * self.row_bytes)
    ...
    def logits_chunked(
        ...
        with open(self.model_file, "rb") as f:
            for start in range(0, self.vocab, chunk_rows):
                ...
                block = np.empty((n, self.hidden), dtype=np.float64)

                for r in range(n):
                    f.seek(self.offset + (start + r) * self.row_bytes)

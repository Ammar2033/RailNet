...
class CompiledTensor:
    def __init__(self, artifact_path, raw_bits=None, shape=None):
        ...
        self.rails_f64 = bf16_array_to_float32(np.array(data["rails"], dtype=np.uint16)).astype(np.float64)
        ...
        if "compressed_routes" in data:
            ...
            self.route_ids = decompress_route_ids(data["compressed_routes"]).astype(np.int32).reshape(shape)
        else:
            if raw_bits is None:
                raise ValueError("raw_bits must be provided if compressed_routes is missing from artifact")

...
def test_entropy_compression_lossless():
    ...
    route_ids = np.random.choice([0, 1, 2, 5, 10, 100, 200], size=1024 * 1024, p=[0.5, 0.2, 0.1, 0.1, 0.05, 0.025, 0.025]).astype(np.uint16)
    ...
    assert compressed_size < uncompressed_size, f"Compression failed to reduce size: {compressed_size} >= {uncompressed_size}"
